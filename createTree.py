...
def writeTree(listOfAllPrev):
    ...
    for prevList in listOfAllPrev:
        for i in range(1, len(prevList)):
            print("node public name: " + str(i))
            print(("node identifier: " + prevList[i] + str(i)))
            if prevList[i] + str(i) not in nodeDict:
                nodeDict[prevList[i] + str(i)] = set()
            if prevList[i-1] + str(i-1) not in nodeDict[prevList[i] + str(i)]:
                tree.create_node(prevList[i], (prevList[i] + str(i)), parent=(prevList[i-1] + str(i-1)))
                nodeDict[prevList[i] + str(i)].add(prevList[i-1] + str(i-1))
